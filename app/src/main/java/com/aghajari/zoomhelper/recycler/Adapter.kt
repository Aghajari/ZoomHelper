...
class Adapter(private val data:List<Model>) : RecyclerView.Adapter<Adapter.ViewHolder>(),ZoomHelper.OnZoomStateChangedListener {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val inflatedView = LayoutInflater.from(parent.context).inflate(R.layout.rv_item,parent, false)
    ...
    override fun onZoomStateChanged(zoomHelper: ZoomHelper, zoomableView: View, isZooming: Boolean) {
        if (isZooming){
            val model: Model = ZoomHelper.getZoomableViewTag(zoomableView) as Model
            Toast.makeText(zoomableView.context,model.name+"'s post started zooming!",Toast.LENGTH_SHORT).show()
        }
    }
}